...
const WEBSITE_KNOWLEDGE = {
    ...
    companyInfo: {
        name: "Onyx", founder: "Raj (RJ)", role: "Software developer, tech researcher",
...
function buildSystemInstruction() {
    ...
    let s = `You are Syncro, the friendly AI assistant for Onyx. Format your responses using markdown: use **bold** for emphasis, bullet points for lists, and \`code\` for technical terms.\n\n`;
    s += `=== WEBSITE KNOWLEDGE ===\n`;
...
// ── Send to AI (with conversation memory baked into message) ─
async function sendToAI(message) {
    showTyping();

    // Build context string from history so /api/chat still works unchanged
    ...
    try {
        ...
        // chat.js wraps reply in candidates format
        ...
        return `😔 Oops! I couldn't connect right now. Please check your connection and try again.\n\n*(${error.message})*`;
    } finally {
        hideTyping();
    }
}
